...
unsigned char galois_mul(unsigned char g1, unsigned char g2)
{
    ...
    {
        if (g2 & 0x01)
        {            // If LSB is active (equivalent to 1 in polynomial of g2)
            p ^= g1; // p += g1 in GF(2^8)
        }
                         ...
                         unsigned char **out)
{
    ...
    if (mode == AES_CTR)
    {
        counter = (unsigned char *)malloc(BLOCK_LEN * sizeof(unsigned char));
        memcpy(counter, iv, BLOCK_LEN * sizeof(unsigned char));

        tmp = (unsigned char *)malloc(BLOCK_LEN * sizeof(unsigned char));
    }
                ...
                unsigned char **out)
{
    ...
    unsigned char(*subkeys)[BLOCK_SIDE][BLOCK_SIDE] = (unsigned char(*)[BLOCK_SIDE][BLOCK_SIDE])malloc((nr + 1) * sizeof(unsigned char[BLOCK_SIDE][BLOCK_SIDE]));
                         ...
                         unsigned char **out)
{
    ...
    padded_out = (unsigned char *)malloc(n_blocks * BLOCK_LEN * sizeof(unsigned char));
    ...
    *out = (unsigned char *)malloc((n_blocks * BLOCK_LEN - no_padding) * sizeof(unsigned char));
    if (*out == NULL)
    {
        return -1;
    }
